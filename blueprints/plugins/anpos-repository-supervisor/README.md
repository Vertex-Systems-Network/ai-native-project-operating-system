...
Full-mode plan generation is source-implemented. Conflict-free non-empty plans use `sandbox_full_plan_v1`. Verified empty repositories can use `guarded_empty_repository_v1` only after explicit confirmation: sandbox transformation occurs first, then one deterministic inert `.anpos-bootstrap-seed` root commit initializes GitHub, the server verifies it has zero parents and is the default-branch head, and the full ANPOS `anpos/*` feature-branch commit deletes the seed before PR/CI review. Any post-seed failure enters recovery-required state. Conflict-bearing plans remain blocked.
