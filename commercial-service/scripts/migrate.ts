...
async function main(): Promise<void> {
  ...
  console.log(
    `migration connection source=${
      process.env.DATABASE_URL_UNPOOLED?.trim() ? "DATABASE_URL_UNPOOLED" : "DATABASE_URL"
    }`,
  );
  ...
  try {
    ...
    await client
      .query("SELECT pg_advisory_unlock(hashtext('anpos-commercial-migrations'))")
      .catch(() => undefined);
    client.release();
    await pool.end();
  }
...
main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "unknown commercial migration failure";
