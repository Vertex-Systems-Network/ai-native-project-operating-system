# ANPOS 1.4.0 / Commercial Service 0.4.10

Status: **customer-facing release draft; not publication evidence**.

Commercial service 0.4.10 adds the source implementation of the Vercel-native signed Repository Supervisor sandbox gateway at `/v1/execute`. The gateway verifies exact-body HMAC requests, enforces durable PostgreSQL nonce replay defense, creates a Python 3.13 Vercel Sandbox Firecracker microVM with `deny-all` networking, rejects secret/environment forwarding, verifies signed input artifacts, executes the current full-plan path in an explicit-empty ephemeral workspace, returns only exact allowlisted SHA-256-verified outputs, and emits a signed success response only after the sandbox workspace is stopped. The current live execution budget is intentionally 240 seconds inside a 300-second function budget; protocol-level `github_commit` workspace mode and the wider 900-second protocol maximum remain fail-closed/pending separate live certification.


Commercial service 0.4.10 adds a strictly internal, read-only Repository Supervisor E2E grant for production verification before real paid Marketplace plans are live. The grant is bound to one GitHub login, requires an explicit expiry no more than 48 hours ahead, never creates or simulates a Marketplace purchase, never grants write/planning capability, and causes full commercial launch readiness to fail closed while configured. It exists only to verify the live read chain `profile -> billing discovery -> resolve -> audit -> assurance` and must be removed after the E2E.

Commercial service 0.4.10 adds principal-scoped Repository Supervisor billing-account discovery. The MCP server returns only the authenticated GitHub user's own entitlement account plus organization entitlement accounts backed by that principal's active seat assignment and current GitHub membership. Capability decisions are derived from the server-side entitlement ledger; the returned non-secret `billing_account_id` remains a selector and never becomes authorization by itself. Global billing-account enumeration is forbidden.

Commercial service 0.4.10 retains sandbox-backed apply for conflict-free non-empty Repository Supervisor plans and adds the guarded empty-repository initialization path described below. Verified private-template Git blobs are re-materialized server-side, staged into a signed remote-ephemeral artifact channel, transformed only inside an isolated Python 3.12+ sandbox, SHA-256/output-path verified on return, and committed to one expected-head feature branch without forwarding the customer GitHub token to the sandbox gateway. Full-plan merge additionally requires a persisted sandbox receipt digest.

Commercial service 0.4.10 also implements guarded `bootstrap_empty` source behavior for GitHub's empty-repository constraint. After explicit confirmation and sandbox verification, the service creates one deterministic inert `.anpos-bootstrap-seed` root commit through the Contents API, verifies zero parents and exact default-branch head, creates the full ANPOS feature-branch commit on that seed while deleting the seed, and retains PR/CI/merge gates. Any post-seed failure becomes recovery-required rather than silently resetting the plan.
...
- commercial service 0.4.10 source identity;
