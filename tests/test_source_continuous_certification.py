...
READINESS_WORKFLOW_PATH = ".github/workflows/commercial-production-readiness.yml"
...
class SourceContinuousCertificationTests(unittest.TestCase):
    ...
    def test_guarded_commercial_production_readiness_audit(self):
        source = subprocess.run(
            ["git", "show", f"HEAD:{READINESS_WORKFLOW_PATH}"],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
