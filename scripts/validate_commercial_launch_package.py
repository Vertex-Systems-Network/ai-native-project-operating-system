...
def main() -> int:
    for path in (APP_BLUEPRINT, COMPLIANCE, CHECKLIST, LISTING, LEGAL, VERIFIER, TESTS, VENDOR_QUALITY, BOUNDARY):
        if not path.is_file():
            fail(f"missing {path.relative_to(ROOT)}")
    ...
    if len(official_sources) < 5 or not all(isinstance(url, str) and url.startswith("https://docs.github.com/") for url in official_sources):
