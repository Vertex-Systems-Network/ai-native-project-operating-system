...
def supervisor_registration_url(inputs: Inputs) -> str:
    ...
    params = [
        ...
        ("description", "ANPOS Repository Supervisor customer repository access application"),
        ("url", inputs.homepage_url),
        ("public", "true"),
        ("webhook_active", "false"),
        ("request_oauth_on_install", "false"),
...
def render(
    ...
    callback_url = inputs.service_base_url + "/api/auth/github/callback"
    supervisor_callback_url = inputs.service_base_url + "/api/auth/mcp/github/callback"
    setup_url = inputs.service_base_url + "/setup/github"
    ...
    return {
        ...
        "github_apps": {
            ...
            "repository_supervisor": {
                ...
                "github_app_webhook_active": False,
                "callback_url": supervisor_callback_url,
                "request_oauth_on_install": False,
                "events": [],
                "permissions": {
                    "metadata": "read",
                ...
                "mcp_auth_flow": "mcp_oauth_broker_then_supervisor_github_app_oauth",
                "generic_write_boundary": "active_project_only_anpos_feature_branch",
        ...
        "operator_sequence": [
            ...
            "Register the public Repository Supervisor App using the prefilled URL; keep ordinary GitHub App webhooks disabled, configure the MCP GitHub OAuth callback, and verify only Metadata read, Contents write, Pull requests write, and Checks read are requested.",
            "Install the Repository Supervisor App only on customer repositories explicitly authorized for supervision; configure its distinct GITHUB_SUPERVISOR_APP_ID/GITHUB_SUPERVISOR_CLIENT_ID/GITHUB_SUPERVISOR_CLIENT_SECRET and do not reuse Marketplace OAuth credentials.",
            "Verify guarded Repository Supervisor writes create only anpos/* feature branches, open PRs, inspect exact-commit checks, respect GitHub branch/review policy, and require expected-head confirmation before merge.",
        ...
        "safety": [
            ...
            "Do not reuse App IDs or OAuth clients across Marketplace, Repository Supervisor, and Vendor Distribution roles.",
            "Do not broaden the Community Marketplace App from its exact ten-file read boundary merely to support Repository Supervisor writes.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        inputs = Inputs(
            ...
            supervisor_app_name=validate_app_name(args.supervisor_app_name, "Repository Supervisor App name"),
