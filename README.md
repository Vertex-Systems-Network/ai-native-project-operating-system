...
- commercial service 0.4.6 deterministic full Repository Supervisor planner for `bootstrap_empty`, `bootstrap_child`, `adopt_existing`, `repair_partial`, and `upgrade_active`, bound to verified private-template release Git identities and immutable target trees; target-only files and Requirements 83–96 project evidence are preserved;
- commercial service 0.4.6 sandbox-backed full-plan apply for **non-empty conflict-free** plans: exact verified private-template Git blobs are staged into a signed remote-ephemeral artifact channel, Python 3.12+ bootstrap transforms execute only inside the sandbox, exact output paths/SHA-256 are verified, customer GitHub credentials are not forwarded to the gateway, and the server creates one expected-head feature-branch commit with a persisted sandbox receipt;
...
This is the **Repository Supervisor source runtime + full planner + conditional sandbox-backed full apply + production E2E harness**, not evidence that the sandbox gateway or public plugin is deployed or that live GitHub read/write E2E has passed. Conflict-resolution apply, guarded empty-repository initialization, live sandbox/E2E receipts and GitLab parity remain subsequent milestones.
