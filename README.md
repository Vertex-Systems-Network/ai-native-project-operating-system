...
- protected sandbox live-probe hardening keeps the signing secret inside Vercel, forwards Vercel OIDC only to the exact configured same-origin gateway, and source-certifies a dedicated trusted-OIDC workflow that verifies runtime/network isolation, HMAC response integrity, exact artifacts, destruction evidence and durable replay rejection before live gateway evidence can be claimed;
