...
- commercial service 0.4.5 deterministic full Repository Supervisor planner for `bootstrap_empty`, `bootstrap_child`, `adopt_existing`, `repair_partial`, and `upgrade_active`, bound to verified private-template release Git identities and immutable target trees; target-only files and Requirements 83–96 project evidence are preserved, while full-mode apply remains fail-closed pending sandbox-backed execution;
- commercial service 0.4.5 signed remote-ephemeral sandbox driver source with immutable GitHub repository/commit workspace binding, HTTPS-only request transport, exact-body HMAC request/response authentication, replay metadata, network deny and workspace-destruction evidence;
...
This is the **Repository Supervisor source runtime + full no-write planner + production sandbox/E2E harness**, not evidence that the sandbox gateway or public plugin is deployed or that live GitHub read/write E2E has passed. Sandbox-backed full-plan apply, live sandbox/E2E receipts and GitLab parity remain subsequent milestones.
